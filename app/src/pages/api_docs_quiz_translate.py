...
#@st.cache_data
with open("/app/src/docs/quiz/api_docs_translation.yml") as f:
...
with col_1:
    st.markdown("""<div style="font-size:26px;font-weight:bold;font-family:'Gothic A1';"> 한국어 퀴즈 번역 API </div>""", unsafe_allow_html=True)
with col_2:
...
with col2:
    with st.container(border=True):
        st.markdown("<h4>한국어 퀴즈 번역</h4>", unsafe_allow_html=True)
        ...
        st.markdown(f"""
                    ...
                    <tr>
                        <td> {docs["method"]} </td>
                        <td> {SERVER_HOST}{docs["url"]}_batch </td>
                        <td> {docs["auth_method"]} </td>
                    </tr>
                    <tr>
                        <td> {docs["method"]} </td>
                        <td> {SERVER_HOST}{docs["url"]}_stream </td>
                        <td> {docs["auth_method"]} </td>
                    </tr>
                    </table>
                    """
        ...
        code_shellSession = f"""
        curl -v -X POST {SERVER_HOST}/api/v1/quiz/translation_{{batch or stream}} \\
        ...
        try:
            response = requests.request(
                method="post",
                url= "{SERVER_HOST}/api/v1/quiz/translation_{{batch or stream}}",
                headers = {{
